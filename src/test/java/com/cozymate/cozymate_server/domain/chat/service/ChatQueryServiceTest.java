    ...
    class getChatList_메서드는 {
        ...
        class memberId와_chatRoomId가_유효한_경우 {
            ...
            void it_returns_chat_list() {
                ChatListResponseDTO chatListResponseDTO = chatQueryService.getChatList(me,
                    chatRoom.getId());

                List<ChatContentResponseDTO> result = chatListResponseDTO.content();

                assertThat(result.size()).isEqualTo(2);
                assertThat(result.get(0).nickname()).isEqualTo(me.getNickname() + " (나)");
                assertThat(result.get(0).content()).isEqualTo(chat.getContent());
                assertThat(result.get(1).nickname()).isEqualTo(you.getNickname());
                assertThat(result.get(1).content()).isEqualTo(chat2.getContent());
            }
        }
        ...
        class 내가_해당_방을_나간_후에_쪽지가_오면 {
            ...
            void it_returns_chat_list_after_delete() {
                ...
                LocalDateTime parsedDateTime = LocalDateTime.parse(result.get(0).datetime(),
                    formatter);

                assertThat(result.size()).isEqualTo(1);
                assertThat(result.get(0).nickname()).isEqualTo(you.getNickname());
                assertThat(result.get(0).content()).isEqualTo(chat3.getContent());
